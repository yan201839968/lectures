...
categoricals <- c() # need to specify which features are categorical (transforms from numeric)
...
random_search_lgbm <- function(parameter_values_fixed,
  ...
  for (ii in 1:n_draws) {
    ...
    search_output[ii, "num_leaves"] <- round(runif(1, min = 2, max = 200))
                                        # round to achieve integer values
...
# if we find the best parameters are close to our boundary min and max values set, need to increase/decrease grid search to see if there are better score outside currently chosen space
```
